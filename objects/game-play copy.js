...
export class GamePlay extends Phaser.GameObjects.Container {
    ...
    update() {
        ...
        if (this.curentSprite && this.dragging && this.clicked) {
            ...
            const draggingSprite = this.curentSprite;
            const targetSprite = draggingSprite.target;

            const getCorners = (sprite) => {
                const w = sprite.displayWidth;
                const h = sprite.displayHeight;
                const cx = sprite.x;
                const cy = sprite.y;
                const rad = Phaser.Math.DegToRad(sprite.angle);
                ...
                return [
                    { x: cx + (cos * -dx - sin * -dy), y: cy + (sin * -dx + cos * -dy) }, // top-left
                    { x: cx + (cos * dx - sin * -dy), y: cy + (sin * dx + cos * -dy) }, // top-right
                    { x: cx + (cos * dx - sin * dy), y: cy + (sin * dx + cos * dy) }, // bottom-right
                    { x: cx + (cos * -dx - sin * dy), y: cy + (sin * -dx + cos * dy) } // bottom-left
                ];
            };

            const shapePoly = getCorners(draggingSprite);
            const targetPoly = getCorners(targetSprite);

            const angleMatch = () => {
                const spriteAngle = Phaser.Math.Angle.WrapDegrees(draggingSprite.angle);
                const targetAngle = Phaser.Math.Angle.WrapDegrees(targetSprite.angle);
                return Math.abs(Phaser.Math.Angle.ShortestBetween(spriteAngle, targetAngle)) <= 10;
            };

            const positionMatch = () => {
                const dist = Phaser.Math.Distance.Between(draggingSprite.x, draggingSprite.y, targetSprite.x, targetSprite.y);
                return dist <= 20;
            };

            const overlap = this.polygonsOverlap(shapePoly, targetPoly);

            if (overlap && positionMatch() && angleMatch()) {
                draggingSprite.alpha = 1;
            } else {
                draggingSprite.alpha = 0.7;
            }

            console.log({
                overlap: overlap,
                posMatch: positionMatch(),
                angleMatch: angleMatch(),
                spriteAngle: draggingSprite.angle,
                targetAngle: targetSprite.angle,
                distance: Phaser.Math.Distance.Between(draggingSprite.x, draggingSprite.y, targetSprite.x, targetSprite.y)
            });
    ...
    polygonsOverlap(polyA, polyB) {
        const polygonA = new Phaser.Geom.Polygon(polyA.map(p => new Phaser.Geom.Point(p.x, p.y)));
        const polygonB = new Phaser.Geom.Polygon(polyB.map(p => new Phaser.Geom.Point(p.x, p.y)));
        ...
        for (let i = 0; i < pointsA.length; i++) {
            ...
            for (let j = 0; j < pointsB.length; j++) {
                ...
                if (Phaser.Geom.Intersects.LineToLine(
                        new Phaser.Geom.Line(a1.x, a1.y, a2.x, a2.y),
                        new Phaser.Geom.Line(b1.x, b1.y, b2.x, b2.y)
                    )) {
                    return true;
                }
            }
        ...
        return (
            pointsA.some(p => Phaser.Geom.Polygon.Contains(polygonB, p)) ||
            pointsB.some(p => Phaser.Geom.Polygon.Contains(polygonA, p))
        );
